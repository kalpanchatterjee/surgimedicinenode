...
router.post("/register", (req, res) => {
  ...
  role_id = role_id == null ? 2 : role_id;
  ...
  db.query(searchsql, function (err, countres) {
    if (countres[0].count == 0) {
      bcrypt.hash(password, saltRounds, (err, hash) => {
        var query =
          "INSERT INTO co_users (first_name, last_name,email,phone,password,role_id) VALUES (?,?,?,?,?,?)";
        db.query(
          query,
          [first_name, last_name, email, phone, hash, role_id],
          function (err, result) {
            if (err) throw err;
